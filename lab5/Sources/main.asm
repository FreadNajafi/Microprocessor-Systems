...
NOT_FWD       CMPA  #REV                      ; Else if it's the REVERSE state          REVERSE STATE ADDED
              BNE   NOT_REV
              JSR   REV_ST                    ; then call the REVERSE routine
              JMP   DISP_EXIT                 ; and exit                                Goal: Displaying all movements of EEbot
              
NOT_REV       CMPA  #ALL_STP                  ; Else if it's the ALL_STOP state         ALL STOP STATE ADDED
              BNE   NOT_ALL_STP
              JSR   ALL_STP_ST                ; then call the ALL_STOP routine
              JMP   DISP_EXIT                 ; and exit
              
NOT_ALL_STP   CMPA  #FWD_TRN                  ; Else if it's the FORWARD_TURN state     FRONT TURN STATE ADDED
              BNE   NOT_FWD_TRN
...
;*******************************************************************
START_ST      BRCLR PORTAD0,$04,NO_FWD        ; If FWD_BUMP (Bumper switch)
...
int2BCD       XGDX                            ; Save the binary number into .X
              ...
              STAA  UNITS
              STAA  BCD_SPARE
              STAA  BCD_SPARE+1               ;Additional space for the decimal point and string terminator
...
;*******************************************************************
;*         Update Display (Battery Voltage and Current State)        *
;*******************************************************************
